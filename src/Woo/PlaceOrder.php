...
class PlaceOrder {
	...
	private function saveOrderResponseToMeta( int $order_id, array $payload, array $response ): void {
		...
		// Save the full ERP payload
		$order->update_meta_data( '_erp_order_payload', maybe_serialize( $payload ) );

		// Save the ERP API response
		$order->update_meta_data( '_erp_order_response', maybe_serialize( $response ) );

		// Optionally, save the ERP reference separately for quick access
